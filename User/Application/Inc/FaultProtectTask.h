...
typedef struct
{
    uint16_t OV_Protect; // 充电过压保护阈值(mV)
    uint16_t OV_Relieve; // 充电过压解除保护阈值(mV)
    uint16_t OV_Delay;   // 充电过压保护延迟时间(s)

    uint16_t UV_Protect; // 放电欠压保护阈值(mV)
    uint16_t UV_Relieve; // 放电欠压解除保护阈值(mV)
    uint16_t UV_Delay;   // 放电欠压保护延迟时间(s)

    uint16_t OCC_Protect;     // 充电过流保护阈值(mA)
    uint16_t OCC_RelieveTime; // 充电过流解除保护时间(s)
    uint16_t OCC_Delay;       // 充电过流保护延迟时间(s)

    uint16_t OCD_Protect;     // 放电过流保护阈值(mA)
    uint16_t OCD_RelieveTime; // 放电过流解除保护时间(s)
    uint16_t OCD_Delay;       // 放电过流保护延迟时间(s)

    uint16_t SCD_Protect;     // 放电短路保护阈值(mA)
